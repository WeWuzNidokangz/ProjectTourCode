const Formats = [
      ...
      for (const set of team) {
        ...
        if (names.has(this.dex.toID(name))) {
          return [
            ...
            `(You have more than one Pok\xE9mon named '${name}')`
          ];
        }
    ...
    validateSet(set, teamHas) {
      ...
      if (!teamHas.abilityMap) {
        ...
        for (const pokemon of Dex.species.all()) {
          if (pokemon.isNonstandard || unreleased(pokemon) && !this.ruleTable.has("+unobtainable"))
            continue;
      ...
      const validSources = teamHas.abilitySources[this.dex.toID(set.species)] = [];
      let canonicalSource = "";
  ...
  },
  {
    ...
    restricted: [
      ...
      "Water Bubble",
      "Wonder Guard",
      "Emergency Exit + Regenerator",
      "Wimp Out + Regenerator"
    ],
    validateSet(set, teamHas) {
      const ability = this.dex.abilities.get(set.ability);
      ...
      if (ability.id === "regenerator" && ["emergencyexit", "wimpout"].includes(item.id) || item.id === "regenerator" && ["emergencyexit", "wimpout"].includes(ability.id)) {
    ...
    field: {
      suppressingWeather() {
        for (const pokemon of this.battle.getAllActive()) {
          ...
          if (pokemon && !pokemon.ignoringAbility() && (pokemon.getAbility().suppressWeather || item.exists && item.suppressWeather)) {
            return true;
          }
        }
        return false;
    ...
    pokemon: {
      ...
      ignoringAbility() {
        ...
        for (const pokemon of this.battle.getAllActive()) {
          if ((pokemon.ability === "neutralizinggas" || pokemon.item === "neutralizinggas") && !pokemon.volatiles["gastroacid"] && !pokemon.abilityState.ending) {
            neutralizinggas = true;
            break;
          }
        }
        return !!(this.battle.gen >= 5 && !this.isActive || (this.volatiles["gastroacid"] || neutralizinggas && this.ability !== "neutralizinggas" && this.item !== "neutralizinggas") && !this.getAbility().isPermanent);
      },
      ignoringItem() {
        let nGas = false;
        for (const pokemon of this.battle.getAllActive()) {
          if ((pokemon.ability === "neutralizinggas" && !pokemon.abilityState.ending || pokemon.item === "neutralizinggas" && !pokemon.itemState.ending) && !pokemon.volatiles["gastroacid"] && !pokemon.abilityState.ending) {
            nGas = true;
            break;
          }
        }
        const item = this.battle.dex.abilities.get(this.item);
        return !!(this.battle.gen >= 5 && !this.isActive || this.hasAbility("klutz") && !this.getItem().ignoreKlutz || this.volatiles["embargo"] || this.battle.field.pseudoWeather["magicroom"] || item.exists && item.id !== "neutralizinggas" && (nGas || this.volatiles["gastroacid"]));
      },
      takeItem(source) {
        if (!this.isActive)
          return false;
