const Formats = [
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			if (!(species.name === 'Mawile' && ability.name === 'Huge Power')) {
				...
				if (_optionalChain([fakeValidation, 'optionalAccess', _ => _.length])) return fakeValidation;
				set.ability = abil;
				return null;
			}
		},
	...
	},
	{
		...
		onModifySpecies(species, target, source) {
			if (source || !_optionalChain([target, 'optionalAccess', _2 => _2.side])) return;
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				const item = this.dex.getItem(set.item);
				if (!_optionalChain([item, 'optionalAccess', _3 => _3.megaStone])) continue;
				const species = this.dex.getSpecies(set.species);
	...
	},
	{
		...
		checkCanLearn(move, species, lsetData, set) {
			// @ts-ignore
			if (!_optionalChain([set, 'access', _4 => _4.sp, 'optionalAccess', _5 => _5.exists]) || !_optionalChain([set, 'access', _6 => _6.crossSpecies, 'optionalAccess', _7 => _7.exists])) {
		...
		validateSet(set, teamHas) {
			...
			let problems = _optionalChain([onChangeSet, 'optionalAccess', _8 => _8.call, 'call', _9 => _9(this, set, this.format)]) || null;
	...
	},
	{
		...
		onValidateTeam(team, f, teamHas) {
			...
			for (const set of team) {
				const abilitySources = _optionalChain([teamHas, 'access', _10 => _10.abilitySources, 'optionalAccess', _11 => _11[this.dex.toID(set.species)]]);
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			...
			if (_optionalChain([status, 'optionalAccess', _12 => _12.exists])) {
		...
		onFaint(target, source, effect) {
			if (_optionalChain([effect, 'optionalAccess', _13 => _13.effectType]) !== 'Move') return;
	...
	},
	{
		...
		onValidateSet(set) {
			...
			if (species.baseSpecies === item.megaEvolves || (item.onPrimal && _optionalChain([item, 'access', _14 => _14.itemUser, 'optionalAccess', _15 => _15.includes, 'call', _16 => _16(species.baseSpecies)])) ||
